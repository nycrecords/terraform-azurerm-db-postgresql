...
resource "null_resource" "db_users" {
  ...
  provisioner "local-exec" {
    ...
    command = "../scripts/ansible-runner.sh ${each.value} ${azurerm_postgresql_server.postgresql_server.fqdn} ${var.administrator_login}@${replace(azurerm_postgresql_server.postgresql_server.fqdn, ".postgres.database.azure.com", "")} ${var.administrator_password}  ${random_password.db_passwords[each.value].result}"
